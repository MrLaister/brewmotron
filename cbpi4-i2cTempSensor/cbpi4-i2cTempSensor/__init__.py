...
@parameters([#Property.Text(label="i2c address", configurable=True, description="Enter the i2c address of your Temp Sensor"),
             Property.Select(label="Channel", options=[0,1,2,3], description="Choose the channel of your Temp Sensor (0-3)"),
             Property.Text(label="Resistor Value", configurable=True, description="Enter the resistor network value"),
             Property.Text(label="Thermistor Nominal Resistance", configurable=True, description="Enter the thermistor nominal resistance e.g. 10000"),
             Property.Text(label="Thermistor Nominal Resistance Temperature", configurable=True, description="Enter the thermistor nominal resistance temperature e.g. 25"),
             Property.Text(label="Thermistor Beta", configurable=True, description="Enter the thermistor beta (Typically 25/85) value e.g. 3435"),
             Property.Text(label="Sample Interval Time - Fast", configurable=True, description="Enter the fast sample interval time e.g. 1"),
             Property.Text(label="Sample Interval Time - Slow", configurable=True, description="Enter the slow sample interval time e.g. 30"),
             Property.Text(label="VMax", configurable=True, description="Enter max voltage (e.g. 3.3)"),
             Property.Kettle(label="Kettle", description="Select the kettle associatated to use faster interval"),
             Property.Fermenter(label="Fermenter", description="Select the fermenter associatated to use faster interval")
             ])
# @parameters([])
...
class i2cTempSensor(CBPiSensor):
    ...
    async def readadc(self):
        if self.channelSelect(self.channel) is None: return 0
    ...
    def channelSelect(self, channel):
        if channel == 0: return ADS.P0
        if channel == 1: return ADS.P1
        if channel == 2: return ADS.P2
        if channel == 3: return ADS.P3
        return None

    def tempConversion(self):
        #Network assumes resistor to Vmax, with NTC probe to gnd
        if self.lastvoltage > self.vmax: self.lastvoltage = self.vmax - 0.00001
