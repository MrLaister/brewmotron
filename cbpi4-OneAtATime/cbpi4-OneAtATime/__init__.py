...
@parameters([
    ...
    Property.Select(label="OneAtATime group", options=[1,2,3,4,5], description="Select a group which this Actor belongs to where only one will be on at a time")
    ])
...
class OneAtATime(CBPiActor):
    async def on_start(self):
        ...
        for actor in self.actors: logger.info(actor)
        logger.info("------------")
        await self.off()
    ...
    async def on(self, input):
        ...
        try:
            if self.cbpi.actor.find_by_id(self.id).instance.running == True:
                ...
                self.state = True
                if self.power == None: self.power = 100
