...
@parameters([
    ...
    Property.Actor(label="Ferment_State", description="Select a GPIO Actor showing Ferment Mode")
    ])
...
class BMTKey(CBPiExtension):
    ...
    async def disableHPActors(self):
        ...
        for actor in self.actors:
            ...
            if self.cbpi.actor.find_by_id(actor).instance.running == True: await self.cbpi.actor.stop(actor)
    
    async def enableHPActors(self):
        print("Enabling OneAtATime Actors")
        self.loadActorValues("OneAtATimeActor")
        for actor in self.actors:
            if self.cbpi.actor.find_by_id(actor).instance.running == False: await self.cbpi.actor.start(actor)
    ...
    async def check_state(self):
        ...
        if truecount == 0: print("WARNING - No key states detected")
        elif truecount == 1:
            self.newMode = newModeList[0]
            if self.mode != self.newMode:
                self.mode = self.newMode
                self.enableMode(self.mode)
        elif truecount > 1: print("WARNING - Multiple key states detected")
