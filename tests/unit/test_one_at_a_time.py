...
class TestOneAtATime:
    ...
    async def test_plugin_initialization(self, plugin_harness, coordination_config, mock_gpio):
        ...
        plugin = await plugin_harness.load_plugin(
            MockOneAtATime,
            coordination_config.id,
            coordination_config.props
        )
    ...
    async def test_coordination_prevents_simultaneous_operation(self, plugin_harness, mock_gpio):
        ...
        actor_configs = [
            GPIOActorConfigFactory(
                id="heater1",
                props={'GPIO': 18, 'CoordinationGroup': 'heaters'}
            ),
            GPIOActorConfigFactory(
                id="heater2", 
                props={'GPIO': 19, 'CoordinationGroup': 'heaters'}
            ),
            GPIOActorConfigFactory(
                id="heater3",
                props={'GPIO': 20, 'CoordinationGroup': 'heaters'}
            )
        ]
    ...
    async def test_actor_queue_management(self, plugin_harness, mock_gpio):
        ...
        actor1 = await plugin_harness.load_plugin(
            MockOneAtATime,
            "actor1",
            {'GPIO': 18, 'CoordinationGroup': 'test_group'}
        )
        
        actor2 = await plugin_harness.load_plugin(
            MockOneAtATime,
            "actor2", 
            {'GPIO': 19, 'CoordinationGroup': 'test_group'}
        )
        
        actor3 = await plugin_harness.load_plugin(
            MockOneAtATime,
            "actor3",
            {'GPIO': 20, 'CoordinationGroup': 'test_group'}
        )
    ...
    async def test_different_coordination_groups(self, plugin_harness, mock_gpio):
        ...
        heater1 = await plugin_harness.load_plugin(
            MockOneAtATime,
            "heater1",
            {'GPIO': 18, 'CoordinationGroup': 'heaters'}
        )
        
        pump1 = await plugin_harness.load_plugin(
            MockOneAtATime,
            "pump1",
            {'GPIO': 19, 'CoordinationGroup': 'pumps'}
        )
...
class TestOneAtATimeEdgeCases:
    ...
    async def test_empty_coordination_group(self, plugin_harness, mock_gpio):
        ...
        config = GPIOActorConfigFactory(
            id="test_empty_group",
            props={'GPIO': 18, 'CoordinationGroup': ''}
        )
