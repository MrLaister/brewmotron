...
from tests.fixtures.hardware_mocks import MockSMBus, MockTemperatureSensor, HardwareTestHarness
...
class TestI2CTempSensor:
    ...
    async def test_sensor_offset_calibration(self, plugin_harness, mock_i2c):
        ...
        plugin = await plugin_harness.load_plugin(
            MockI2CTempSensor,
            config_with_offset.id,
            config_with_offset.props
        )
...
class TestI2CTempSensorEdgeCases:
    ...
    async def test_extreme_temperature_readings(self, plugin_harness, mock_i2c):
        ...
        config = PluginConfigFactory(
            id="test_extreme_temps",
            type="Sensor",
            props={'SensorAddress': '0x48'}
        )
