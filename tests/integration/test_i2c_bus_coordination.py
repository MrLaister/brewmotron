...
from tests.fixtures.hardware_mocks import MockSMBus, HardwareTestHarness, Mock7SegmentDisplay
...
class TestI2CBusCoordination:
    ...
    async def test_i2c_transaction_timing(self, i2c_harness):
        ...
        display_updates = [h for h in mash_display.access_history 
                          if h['operation'] == 'display_update']
        sensor_reads = [h for h in temp_sensor.access_history 
    ...
    async def test_i2c_error_recovery(self, i2c_harness):
        ...
        error_entries = [h for h in mash_display.access_history 
                        if 'error' in h['operation']]
    ...
    async def test_cross_device_data_flow(self, i2c_harness):
        ...
        lcd_updates = [h for h in lcd_display.access_history 
                      if h['operation'] == 'lcd_update']
